# CP-SAT 优化器详解 —— 从零读懂 `CpSatOptimizer.Optimize`

## 目录

- [CP-SAT 优化器详解 —— 从零读懂 `CpSatOptimizer.Optimize`](#cp-sat-优化器详解--从零读懂-cpsatoptimizeroptimize)
  - [目录](#目录)
  ...
  - [3. CP-SAT 是什么？为什么用它？](#3-cp-sat-是什么为什么用它)
    - [CP-SAT 全称](#cp-sat-全称)
    - [它能干什么](#它能干什么)
    - [为什么不用普通的线性规划（LP）求解器？](#为什么不用普通的线性规划lp求解器)
    - [为什么不用启发式算法？](#为什么不用启发式算法)
  ...
  - [6. 逐步详解：11 步走完整个优化流程](#6-逐步详解11-步走完整个优化流程)
    ...
    - [步骤 9：成对顺序约束（最复杂的部分）](#步骤-9成对顺序约束最复杂的部分)
      - [9.1 为什么需要成对顺序约束？](#91-为什么需要成对顺序约束)
      - [9.2 数据结构](#92-数据结构)
      - [9.3 "both" 变量：两个 Job 是否都在该机器上](#93-both-变量两个-job-是否都在该机器上)
      - [9.4 顺序变量与排他性](#94-顺序变量与排他性)
      - [9.5 颜色合法性检查与排序约束](#95-颜色合法性检查与排序约束)
      - [9.6 ColorPolicy 的颜色合法性判断](#96-colorpolicy-的颜色合法性判断)
      - [9.7 完整逻辑总结](#97-完整逻辑总结)
    - [步骤 10：目标函数与启发式热启动](#步骤-10目标函数与启发式热启动)
      - [10a. 延误项（Tardiness）](#10a-延误项tardiness)
      - [10b. 完工时间项（Makespan）](#10b-完工时间项makespan)
      - [10c. 组合目标函数](#10c-组合目标函数)
      - [10d. 启发式 Hint（热启动）](#10d-启发式-hint热启动)
    - [步骤 11：求解与提取结果](#步骤-11求解与提取结果)
      - [11a. 创建求解器并求解](#11a-创建求解器并求解)
      - [11b. 提取结果](#11b-提取结果)
      - [11c. 返回结果](#11c-返回结果)
  - [7. CP-SAT API 速查表](#7-cp-sat-api-速查表)
    ...
    - [变量创建](#变量创建)
    - [约束](#约束)
    - [条件约束（OnlyEnforceIf）](#条件约束onlyenforceif)
    - [目标函数](#目标函数)
    - [表达式](#表达式)
    - [热启动](#热启动)
    - [求解器](#求解器)
  ...
  - [8. 常见疑问解答（FAQ）](#8-常见疑问解答faq)
    - [Q1：为什么步骤 9 中的 "both" 变量需要用四条约束而不是一个简单的 API？](#q1为什么步骤-9-中的-both-变量需要用四条约束而不是一个简单的-api)
    - [Q2：为什么 tardiness 用 `>=` 而不是 `==`？](#q2为什么-tardiness-用--而不是-)
    - [Q3：为什么 HorizonMinutes 默认是 2880（48 小时）？](#q3为什么-horizonminutes-默认是-288048-小时)
    - [Q4：Hint 会导致求解器陷入局部最优吗？](#q4hint-会导致求解器陷入局部最优吗)
    - [Q5：20 秒够找到最优解吗？](#q520-秒够找到最优解吗)
    - [Q6：ObjectiveValue 可以用来做什么？](#q6objectivevalue-可以用来做什么)
    - [Q7：为什么 `LinearExpr.Sum` 不用普通的 `+` 运算符？](#q7为什么-linearexprsum-不用普通的--运算符)
    - [Q8：`AddBoolOr` 中的 `leftVars.Present.Not()` 是什么意思？](#q8addboolor-中的-leftvarspresentnot-是什么意思)
    - [Q9：代码中 `[..]` 是什么语法？](#q9代码中--是什么语法)
  - [9. 延伸学习资源](#9-延伸学习资源)
    - [官方文档](#官方文档)
    - [本项目的相关代码](#本项目的相关代码)
    - [学习路径建议](#学习路径建议)
